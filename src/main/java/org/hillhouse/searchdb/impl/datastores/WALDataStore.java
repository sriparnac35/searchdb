...
public class WALDataStore implements DataStore<WalDataKey, WalValue, WalSearchKey, WalSearchValue> {
    ...
    private WALOperationType deriveOperationTypeFromFlag(byte flag){
        switch (flag){
            case VALUE_OPERATION_TYPE_INSERT : return WALOperationType.INSERT;
            case VALUE_OPERATION_TYPE_UPDATE : return WALOperationType.UPDATE;
            case VALUE_OPERATION_TYPE_DELETE : return WALOperationType.DELETE;
            default: return null;
        }
    }
    ...
    private WalCommitStatus deriveCommitStatusFromFlag(byte status){
        switch (status){
            case VALUE_COMMIT_STATE_STARTED : return WalCommitStatus.COMMIT_BEGIN;
            case VALUE_COMMIT_STATE_ENDED: return WalCommitStatus.COMMIT_END;
            case VALUE_COMMIT_STATE_FAILED : return WalCommitStatus.COMMIT_FAILED;
            default : return null;
        }
    }
    ...
    private WalEntryType deriveEntryTypeFromByte(byte type){
        switch (type){
            case VALUE_ENTRY_TYPE_STATE : return WalEntryType.COMMIT_STATE;
            case VALUE_ENTRY_TYPE_DATA : return WalEntryType.DATA;
            default: return null;
        }
    }
    ...
    private byte deriveCommitState(WalCommitStatus commitStatus){
        switch (commitStatus){
            case COMMIT_BEGIN: return VALUE_COMMIT_STATE_STARTED;
            case COMMIT_FAILED: return WalConstants.VALUE_COMMIT_STATE_FAILED;
            case COMMIT_END: return WalConstants.VALUE_COMMIT_STATE_ENDED;
            default: return 0;
        }
    }
    ...
    private byte deriveOperationTypeFlag(WALOperationType operationType){
        switch (operationType){
            case INSERT: return WalConstants.VALUE_OPERATION_TYPE_INSERT;
            case UPDATE: return WalConstants.VALUE_OPERATION_TYPE_UPDATE;
            case DELETE: return WalConstants.VALUE_OPERATION_TYPE_DELETE;
            default: return 0;
        }
    }
